...
const geometryModule = require("./geometry");
...
function geometryToleranceForZoom(zoom) {
  return geometryModule.geometryToleranceForZoom(zoom);
}

function perpendicularDistance(point, start, end) {
  return geometryModule.perpendicularDistance(point, start, end);
}

function simplifyLineStringCoordinates(coords, tolerance) {
  return geometryModule.simplifyLineStringCoordinates(coords, tolerance);
}

function simplifyGeometryForZoom(geometry, zoom) {
  return geometryModule.simplifyGeometryForZoom(geometry, zoom);
}

function geometrySourceHash(geometry) {
  return geometryModule.geometrySourceHash(geometry);
}

async function resolveGeometryForZoom(route, options = {}) {
  return geometryModule.resolveGeometryForZoom(route, options);
}
